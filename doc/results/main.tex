...
Também comparamos os valores numérico da nossa solução com os valores calculados pela abordagem anterior (Ver \figref{fig:coef_plate}, \figref{fig:coef_mug} e \figref{fig:coef_key}). A diferença numérica entre os métodos ficou abaixo de 5\%.
...
Para testar a qualidade do áudio final, um teste A/B foi feito com voluntários. Nesse teste, o voluntário assistia ambas as versões do vídeo (identificadas apenas por A ou B) e deveria responder à pergunta: ``Qual dos dois pareceu mais realista? A ou B?''. O voluntário tinha como opções: ``A'', ``B'' ou ``Não sei''. A \tabref{tab:survey_results} contém o resultado da pesquisa.
